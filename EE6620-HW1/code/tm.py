...
if __name__ == '__main__':
    ...
    cv.imwrite('../Experiments/b/p2_gtm_scale06.png', ldr)

    radiance = cv.imread('../TestImg/memorial.hdr', -1)
    golden = cv.imread('../ref/p2_gtm.png')
    ldr = globalTM(radiance, scale=0.8)
    cv.imwrite('../Experiments/b/p2_gtm_scale08.png', ldr)

    radiance = cv.imread('../TestImg/memorial.hdr', -1)
    golden = cv.imread('../ref/p2_gtm.png')
    ldr = globalTM(radiance, scale=1)
    cv.imwrite('../Experiments/b/p2_gtm_scale10.png', ldr)

    radiance = cv.imread('../TestImg/memorial.hdr', -1)
    golden = cv.imread('../ref/p2_gtm.png')
    ldr = globalTM(radiance, scale=1.2)
    cv.imwrite('../Experiments/b/p2_gtm_scale12.png', ldr)

    radiance = cv.imread('../TestImg/memorial.hdr', -1)
    golden = cv.imread('../ref/p2_gtm.png')
    ldr = globalTM(radiance, scale=1.4)
    cv.imwrite('../Experiments/b/p2_gtm_scale14.png', ldr)

    radiance = cv.imread('../TestImg/vinesunset.hdr', -1)
    golden = cv.imread('../ref/p3_ltm.png')
    gauhw1 = partial(gaussianFilter, N=35, sigma_s=100)
    test = localTM(radiance, gauhw1, scale=2)
    cv.imwrite('../Experiments/b/p3_ltm_scale2.png', test)

    radiance = cv.imread('../TestImg/vinesunset.hdr', -1)
    golden = cv.imread('../ref/p3_ltm.png')
    gauhw1 = partial(gaussianFilter, N=35, sigma_s=100)
    test = localTM(radiance, gauhw1, scale=6)
    cv.imwrite('../Experiments/b/p3_ltm_scale6.png', test)

    radiance = cv.imread('../TestImg/vinesunset.hdr', -1)
    golden = cv.imread('../ref/p3_ltm.png')
    gauhw1 = partial(gaussianFilter, N=35, sigma_s=100)
    test = localTM(radiance, gauhw1, scale=10)
    cv.imwrite('../Experiments/b/p3_ltm_scale10.png', test)